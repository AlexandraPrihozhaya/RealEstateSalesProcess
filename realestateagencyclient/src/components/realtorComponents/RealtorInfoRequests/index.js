...
import { getAllRequests, updateRequest, addNotificationLeadClient, addTransaction } from '../../utils/ApiFunctions';
...
const RealtorInfoRequests = () => {
  ...
  const columns = ['№', 'Объект недвижимости', 'Клиент', 'Дата заявки', 'Статус', ''];

  const [currentPage, setCurrentPage] = useState(1);
  const [itemsPerPage, setItemsPerPage] = useState(10);

  const sortedPageData = requests.sort((a, b) => {
...
const handleUpdate = async (request, status, leadClientId) => {
    try {
        const result = await updateRequest(request.id, status);
        if (result === "Request updated successfully") {
          if (status === true) {
            //если существует клиент
            const result2 = await addTransaction(userId, request.realEstateObject.id, leadClientId);
            const result3 = await addNotificationLeadClient(leadClientId, notificationObjectTrue);
          } else {
              ...
              <STbody>
                  {pageData.map((request, index) => (
                      <STr key={request.id} status={request.status}>
                          ...
                          <STd>
                          <AcceptButton 
                              onClick={() => handleUpdate(request, true, request.leadClient.id)}
                              disabled={request.status !== null} 
                          >
                              Принять
                          </AcceptButton>
                          <RejectButton 
                              onClick={() => handleUpdate(request, false, request.leadClient.id)}
                              disabled={request.status !== null} 
                          >
                              Отклонить
                          </RejectButton>
